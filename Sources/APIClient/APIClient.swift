...
public protocol APIClientConfiguration: AnyObject, Sendable {
    var baseURL: URL { get }
}
...
public protocol APIClient: Sendable {
    ...
    func decode<T: Decodable>(_ type: T.Type, from data: Data, userInfo: [CodingUserInfoKey: any Sendable]?) throws -> T
    ...
    func send<T: Decodable>(_ request: URLRequest, parsing context: (any APIParsingContext)?, completion: @Sendable @escaping (Result<APIResponse<T>, APIClientError>) -> Void)
...
extension APIClient {
    ...
    public func error(from data: Data) -> (any Error)? {
        ...
        return try? jsonDecoder.decode(OktaAPIError.self, from: data)
    }
    ...
    public func send<T: Decodable>(_ request: URLRequest, parsing context: (any APIParsingContext)? = nil, completion: @Sendable @escaping (Result<APIResponse<T>, APIClientError>) -> Void) {
    ...
    private func send<T>(_ request: URLRequest,
                         parsing context: (any APIParsingContext)? = nil,
                         state: APIRetry.State?,
                         completion: @Sendable @escaping (Result<APIResponse<T>, APIClientError>) -> Void) {
        ...
        session.dataTaskWithRequest(urlRequest) { data, response, httpError in
            ...
            do {
                guard let httpResponse = response as? HTTPURLResponse,
                    let httpHeaders = httpResponse.allHeaderFields as? [String: any Sendable]
                else {
                ...
                rateInfo = APIRateLimit(with: httpHeaders)
...
extension APIClient {
    ...
    private func validate<T>(data: Data, response: HTTPURLResponse, rateInfo: APIRateLimit?, parsing context: (any APIParsingContext)? = nil) throws -> APIResponse<T> {
        ...
        if let dateString = response.allHeaderFields["Date"] as? String {
            date = DateFormatter.httpDateFormatter.date(from: dateString)
        }
...
private let _httpDateFormatter: DateFormatter = {
...
private let _defaultIsoDateFormatter: DateFormatter = {
...
private let _defaultAPIClientJSONEncoder: JSONEncoder = {
...
private let _defaultAPIClientJSONDecoder: JSONDecoder = {
...
extension DateFormatter {
    public static var httpDateFormatter: DateFormatter { _httpDateFormatter }
    public static var defaultIsoDateFormatter: DateFormatter { _defaultIsoDateFormatter }
}

extension JSONEncoder {
    public static var apiClientEncoder: JSONEncoder { _defaultAPIClientJSONEncoder }
}

extension JSONDecoder {
    public static var apiClientDecoder: JSONDecoder { _defaultAPIClientJSONDecoder }
}
