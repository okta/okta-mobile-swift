...
public struct JWT: RawRepresentable, Codable, Sendable, HasClaims, Expires {
    ...
    public init(_ token: String) throws {
        ...
        guard let payload = try JSONSerialization.jsonObject(with: payloadData, options: []) as? [String: any Sendable] else {
            throw JWTError.badTokenStructure
        }
    ...
    public static func tokenComponents(from token: String) -> [String] {
        token
