...
extension IDXClient.APIVersion1.AnswerChallengeRequest: IDXClientAPIRequest {
    typealias ResponseType = Decodable

    func urlRequest(using configuration: IDXClient.Configuration) -> URLRequest? {
        guard let url = configuration.issuerUrl(with: "idp/idx/challenge/answer") else { return nil }
        return nil
    }
