...
final class DirectAuth2FASignInTests: XCTestCase {
    lazy var user: TestUser? = {
        let env = ProcessInfo.processInfo.environment
        guard let username = env["E2E_MFA_USERNAME"],
              let password = env["E2E_MFA_PASSWORD"],
              let otpSecret = env["E2E_MFA_OTP_SECRET"]
        else {
            return nil
        }
    ...
    func testPasswordOTP() throws {
        ...
        signInScreen.validate(state: .secondaryFactor)
        signInScreen.login(factor: .otp, value: user.otpSecret)
