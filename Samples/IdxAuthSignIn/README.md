...
Add native sign in to a native application that leverages the Okta Identity Engine to support server-driven policy decisions for authenticating users.
...
1. Open the `OktaClient.xcworkspace` workspace from the root level in Xcode. 
2. Select the `IdxAuthSignIn` application target, and choose a simulator type to use.
