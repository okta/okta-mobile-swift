...
struct DefaultIDTokenValidatorTests {
    ...
    @Test("Null validator context", )
    func testNullValidatorContext() throws {
        #expect(NullIDTokenValidatorContext.nonce == nil)
        #expect(NullIDTokenValidatorContext.maxAge == nil)
    }
    
    @Test("Valid ID token")
    func testValidIDToken() throws {
        let jwt = try JWT(validToken)
        try validator.validate(token: jwt, issuer: issuer, clientId: clientId, context: nil)
    }
    ...
    func testInvalidURLScheme() throws {
        ...
        let error = #expect(throws: JWTError.self) {
            try validator.validate(token: jwt, issuer: URL(string: "http://example.okta.com/oauth2/default")!, clientId: clientId, context: nil)
        }
        
        #expect(error == .issuerRequiresHTTPS)
    }
    ...
    func testUnsupportedAlgorithm() throws {
        ...
        #expect(error == .unsupportedAlgorithm(.rs384))
    }

    @Test("Expired token", .mockTimeCoordinator)
    ...
    @Test("IssuedAt time exceeds grace period", .mockTimeCoordinator(offset: baselineTime))
    func testIssuedAtExceedsGracePeriod() throws {
        let mockTime = try #require(Date.coordinator as? MockTimeCoordinator)
        mockTime.offset += 320
        let jwt = try JWT(validToken)
        let error = #expect(throws: JWTError.self) {
            try validator.validate(token: jwt, issuer: issuer, clientId: clientId, context: nil)
        }
        #expect(error == .issuedAtTimeExceedsGraceInterval)
    }
    ...
    func testNonce() throws {
        ...
        let nonceError = #expect(throws: JWTError.self) {
            try validator.validate(token: jwt, issuer: issuer, clientId: clientId, context: nil)
        }
        #expect(nonceError == .nonceMismatch)
        
        let mismatchError = #expect(throws: JWTError.self) {
            try validator.validate(token: jwt, issuer: issuer, clientId: clientId, context: MockTokenContext(nonce: "does_not_match", maxAge: nil))
        }
        #expect(mismatchError == .nonceMismatch)

        try validator.validate(token: jwt, issuer: issuer, clientId: clientId, context: MockTokenContext(nonce: "B34AAE01-5757-4A0B-9362-B02F6F02A3B1", maxAge: nil))
    }
    ...
    func testInvalidAuthTime() throws {
        ...
        let error = #expect(throws: JWTError.self) {
            try validator.validate(token: jwt, issuer: issuer, clientId: clientId, context: MockTokenContext(nonce: nil, maxAge: 300))
        }
        #expect(error == .invalidAuthenticationTime)
    }
    ...
    func testExceedsMaxAge() throws {
        ...
        let error = #expect(throws: JWTError.self) {
            try validator.validate(token: jwt, issuer: issuer, clientId: clientId, context: MockTokenContext(nonce: nil, maxAge: 200))
        }
        #expect(error == .exceedsMaxAge)
    }
}
