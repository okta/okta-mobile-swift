...
class URLSessionMock: URLSessionProtocol, @unchecked Sendable {
    ...
    func dataTaskWithRequest(_ request: URLRequest, completionHandler: @Sendable @escaping (Data?, URLResponse?, (any Error)?) -> Void) -> any URLSessionDataTaskProtocol {
    ...
    func dataTask(with request: URLRequest, completionHandler: @Sendable @escaping (Data?, URLResponse?, (any Error)?) -> Void) -> any URLSessionDataTaskProtocol {
        let response = call(for: request.url!.absoluteString)
        requests.append(request)
        return URLSessionDataTaskMock(session: self,
                                      data: response?.data,
...
final class URLSessionDataTaskMock: URLSessionDataTaskProtocol {
    nonisolated(unsafe) weak var session: URLSessionMock?
    
    let completionHandler: @Sendable (Data?, HTTPURLResponse?, (any Error)?) -> Void
    let data: Data?
    let response: HTTPURLResponse?
    let error: (any Error)?
    ...
    init(session: URLSessionMock,
         ...
         error: (any Error)?,
         completionHandler: @Sendable @escaping (Data?, HTTPURLResponse?, (any Error)?) -> Void)
    {
